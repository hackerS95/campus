...
// Try removing the loading screen after 2 seconds
setTimeout(() => {
  try {
    removeLoadingScreen();
  } catch (error) {
...
// If not removed after 5 seconds, try again
setTimeout(() => {
  const loadingScreen = document.getElementById('loadingScreen');
  if (loadingScreen) {
    console.log('Loading screen still present, retrying...');
    try {
      removeLoadingScreen();
    } catch (error) {
      console.error('Error removing loading screen on retry:', error);
    }
  } else {
    console.log('Loading screen successfully removed earlier.');
  }
}, 7000);
